...
## Let's go through a very simple example {.flexbox .vcenter .smaller}
...
- Goof practice to write this in the *R* file so that everything is self-contained.
```{r}
modelstring <- paste(
  "model {
...
writeLines(modelstring, con = "example1.txt" )
```
...
reg.jags <- jags.model(file = "example1.txt", data=dat, inits=jags.inits, n.chains=1, n.adapt=1000)
...
## Let's go through a very simple example {.flexbox .vcenter .smaller}
```{r}
reg.df <- tbl_df(as.data.frame(regression.sim[[1]])) 
reg.df <- reg.df %>% gather(coef, value)
ggplot(reg.df, aes(x = value)) + geom_density() +  facet_wrap(~ coef) 
```